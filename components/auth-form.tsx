...
export function AuthForm() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        toast({
          title: "lỗi đăng nhập",
          description: data.error || " đăng nhập thành công",
          variant: "destructive",
        });
        return;
      }
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (registerPassword.length < 6) {
      toast({
        title: "lỗi",
        description: "mật khẩu phải chứa ít nhất 6 kí tự",
        variant: "destructive",
      });
      return;
    }
...
return (
  <Card className="mx-auto w-full max-w-md">
    <CardHeader>
      <CardTitle className="font-serif text-2xl">Tài Khoản Cá Nhân</CardTitle>
